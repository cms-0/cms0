...
Use `/api/health` for load balancers, Docker health checks, and uptime probes.
It only verifies app plus Postgres readiness.

Use `/api/content/health` with a valid API key when you need to verify the
authenticated runtime API surface used by applications and CI.
